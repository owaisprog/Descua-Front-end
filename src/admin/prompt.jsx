...
import PromptsEditModal from '../components/propmtsEditModel'
...
function Prompts() {
  const notify = () => toast("Prompt deleted");
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          "http://localhost:7000/api/getAllPrompts"
        );
  ...
  const handleDelete = async (itemId) => {
    try {
      const response = await fetch(
        `http://localhost:7000/api/deletePrompt/${itemId}`,
        {
          method: "DELETE",
          headers: {
      ...
      <div className="bg-bg-color  ">
        <div className="  max-w-screen-xl mx-auto py-7  px-4 xl:px-0">
          <div className="flex gap-10">
            ...
            <div>
              <div className="w-full">
                <div className="px-4 bg-white py-2  flex justify-between">
                  <h1 className="text-2xl font-bold my-3">Prompts</h1>
                  <div>
                    <button
                ...
                <table className="w-full ">
                  <thead>
                    <tr className="bg-white border  ">
                      ...
                      <th className="  w-[300px] py-6">statement</th>
                      <th className=" w-[300px] py-6">date</th>
            ...
            {editModal && editID && (
               <PromptsEditModal  ID={editID}  CloseEditModal={closeEditModal} EditState={setShowEditModal} />
            ) }
          </div>
        </div>
      </div>
